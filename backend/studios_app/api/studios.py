...
from backend.studios_app.core.studios import StudiosHandle


class StudiosAPIView(APIView):
    def get(self, request, pk=None, **kwargs):
        call_response = StudiosHandle(request)
    ...
    def post(self, request, **kwargs):
        ...
        if call_response[0] == status.HTTP_200_OK:
            call_response = StudiosHandle(request).create()
        return Response({'details': call_response[1], 'data': call_response[2]}, call_response[0])

    def put(self, request, pk, **kwargs):
        call_response = UserAuthenticationManager(request).handle_logged_in()
        if call_response[0] == status.HTTP_200_OK:
            call_response = StudiosHandle(request).update(pk)
        return Response({'details': call_response[1], 'data': call_response[2]}, call_response[0])

    def delete(self, request, pk, **kwargs):
        call_response = UserAuthenticationManager(request).handle_logged_in()
        if call_response[0] == status.HTTP_200_OK:
            call_response = StudiosHandle(request).delete(pk)
        return Response({'details': call_response[1], 'data': call_response[2]}, call_response[0])
